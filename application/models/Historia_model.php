...
class Historia_model extends CI_Model {
    ...
    function historia_informe_excel($fecha_ini,$fecha_fin) {
        ...
        paciente.codiUbi, paciente.municipio,paciente.celular,paciente.email,paciente.cumple, 
        historia.codiag, historia.coproc,sedes.nombre_sede,usuario.nombre1 as n_medico,
        usuario.apellido1 as a_medico ,roles.nombre
        FROM historia 
        JOIN `paciente` ON historia.documento = paciente.ndoc 
                            and historia.tipodoc = paciente.tipodoc 
        JOIN `usuario` ON usuario.documento = historia.id_usuario
        JOIN `roles` ON roles.psips = historia.psips
        JOIN `sedes` ON sedes.id_sede = historia.id_sede
        ...
        paciente.municipio,paciente.celular,paciente.email,paciente.cumple, 
        evolucion.codiag, evolucion.coproc,sedes.nombre_sede,usuario.nombre1 as n_medico,
        usuario.apellido1 as a_medico ,roles.nombre
        FROM evolucion 
        JOIN paciente ON evolucion.ndoc = paciente.ndoc 
                          and evolucion.tipodoc = paciente.tipodoc
        JOIN `usuario` ON usuario.documento = evolucion.id_usuario
        JOIN `roles` ON roles.psips = evolucion.psips 
        JOIN `sedes` ON sedes.id_sede = evolucion.id_sede 
